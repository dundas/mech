...
router.post('/', async (req, res) => {
    try {
        ...
        logger_1.default.error('Error creating subscription:', error);
        res.status(500).json({
            success: false,
...
router.get('/', async (req, res) => {
    try {
        ...
        const subscriptions = await subscriptionService.listSubscriptions(req.application.id, {
        ...
        logger_1.default.error('Error listing subscriptions:', error);
        res.status(500).json({
            success: false,
...
router.get('/:id', async (req, res) => {
    try {
        const subscription = await subscriptionService.getSubscription(req.params.id, req.application.id);
        if (!subscription) {
        ...
        logger_1.default.error('Error getting subscription:', error);
        res.status(500).json({
            success: false,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const subscription = await subscriptionService.updateSubscription(req.params.id, req.application.id, updates);
        if (!subscription) {
            return res.status(404).json({
                success: false,
                error: 'Subscription not found',
...
router.delete('/:id', async (req, res) => {
    try {
        const deleted = await subscriptionService.deleteSubscription(req.params.id, req.application.id);
        if (!deleted) {
            return res.status(404).json({
                success: false,
                error: 'Subscription not found',
        ...
        logger_1.default.error('Error deleting subscription:', error);
        res.status(500).json({
            success: false,
...
router.post('/:id/test', async (req, res) => {
    try {
        ...
        logger_1.default.error('Error testing subscription:', error);
        res.status(500).json({
            success: false,
