...
exports.extractContextFromRequest = extractContextFromRequest;
class AIHelper {
    static generateSuggestion(context) {
        const suggestions = [];
        // Endpoint-specific suggestions
        if (context.endpoint.includes('/jobs') && context.method === 'POST') {
            suggestions.push('Ensure job data includes all required fields', 'Check queue name is correct (case-sensitive)', 'Verify API key has access to this queue');
        }
    ...
    static getErrorSuggestions(error) {
        ...
        if (error.includes('timeout')) {
            suggestions.push('Increase timeout value in job options', 'Check if target service is responding', 'Consider breaking large jobs into smaller ones');
        }
        if (error.includes('validation')) {
            suggestions.push('Use /api/explain endpoint for field requirements', 'Ensure JSON is properly formatted', 'Check data types match expected schema');
        }
        if (error.includes('rate limit')) {
            suggestions.push('Implement exponential backoff', 'Check X-RateLimit headers for reset time', 'Consider batching requests');
        }
        return suggestions;
    }
    static generateExample(queueName) {
        const examples = {
            email: {
    ...
    static explainJobStatus(status) {
        const explanations = {
    ...
    static generateCurlExample(method, endpoint, apiKey = 'your-api-key', data) {
    ...
    static generateSDKExample(language, method, endpoint, data) {
